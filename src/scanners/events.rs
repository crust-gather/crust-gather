        ...
        let row = TableRow::new()
            .with_cell(TableCell::default().with_raw({
                let (creation, first, last) = (
                    event.metadata.creation_timestamp.clone().unwrap_or(Time(DateTime::MIN.to_zoned(TimeZone::UTC)?.timestamp())).0,
                    event.first_timestamp.clone().unwrap_or(Time(DateTime::MIN.to_zoned(TimeZone::UTC)?.timestamp())).0,
                    event.last_timestamp.clone().unwrap_or(Time(DateTime::MIN.to_zoned(TimeZone::UTC)?.timestamp())).0
                );
